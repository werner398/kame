...
router.get("/:sku", (req, res) => {
  const filas = db
    .prepare(
      "SELECT id, proveedor, codigo_proveedor, precio, moneda, cantidad_minima, plazo_entrega, kg_m, activo, usuario, fecha_actualizacion FROM proveedores_producto WHERE sku = ? ORDER BY activo DESC, fecha_actualizacion DESC"
    )
    .all(req.params.sku);
  res.json(filas);
});
...
router.post("/:sku", (req, res) => {
  const { proveedor, codigoProveedor, precio, moneda, kgM, usuario } = req.body;
  ...
  const resultado = db
    .prepare(
      `INSERT INTO proveedores_producto
       (sku, proveedor, codigo_proveedor, precio, moneda, kg_m, usuario, activo)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1)`
    )
    .run(req.params.sku, proveedor, codigoProveedor || null, precio || null, moneda || "CLP", kgM || null, usuario);
...
router.put("/:sku/:id", (req, res) => {
  const { precio, moneda, cantidadMinima, plazoEntrega, activo, usuario } = req.body;
  ...
  db.prepare(
    `UPDATE proveedores_producto
     SET precio = ?, moneda = ?, cantidad_minima = ?, plazo_entrega = ?, activo = ?, usuario = ?, fecha_actualizacion = datetime('now')
     WHERE id = ?`
  ).run(
    precio ?? fila.precio,
    moneda || fila.moneda,
